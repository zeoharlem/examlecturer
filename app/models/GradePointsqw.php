...
class GradePoints extends Phalcon\Mvc\Model{
    ...
    private static $_grades, $_units, $_totalUnits, $_totalUnitsPass, $_totalUnitsFail, $_totalGrade;
    
    //Test the score and assign corresponding grades
    public static function __gradeParser($score,$unit){
        if($score > 69){
            self::$_grades = 7;
        ...
        if($score > 59 && $score < 65){
            self::$_grades = 5;
            self::$_totalGrade[] = self::$_grades*$unit;
            self::__setUnitPF($score, $unit);
        }
        if($score > 54 && $score < 60){
            self::$_grades = 4;
            self::$_totalGrade[] = self::$_grades*$unit;
            self::__setUnitPF($score, $unit);
        }
